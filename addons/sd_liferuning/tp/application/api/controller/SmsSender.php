...
class SmsSingleSender {
    ...
    function send($type, $nationCode, $phoneNumber, $msg, $extend = "", $ext = "") {
        ...
        */
        ...
        $curTime = time();
        $wholeUrl = $this->url . "?sdkappid=" . '1400038619' . "&random=" . $random;
        // 按照协议组织 post 包体
        $data = new \stdClass();
        $tel = new \stdClass();
        ...
        $data->type = (int)$type;
        $data->msg = $msg;
        $data->sig = hash("sha256","appkey="."cd83aabd579774a79be2d9a38278624d"."&random=".$random."&time=".$curTime."&mobile=".$phoneNumber, FALSE);
        $data->time = $curTime;
        $data->extend = $extend;
        $data->ext = $ext;
