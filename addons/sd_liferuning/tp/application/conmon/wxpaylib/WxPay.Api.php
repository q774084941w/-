...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
      ...
      //var_dump($inputObj);die;
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(config('wxpay.WEIXIN_URL'));//异步通知url
		}

        $uniacid=$_GET['_uniacid'];
        $key=db('Business')->field('appid,mchid,key,secret')->where('uniacid',$uniacid)->find();
		...
		//签名
        $inputObj->SetAppid($key['appid']);//公众账号ID
        $inputObj->SetMch_id($key['mchid']);//商户号
		$inputObj->SetSign();
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function refund($bid,$inputObj, $timeOut = 6)
	{
		...
		$xml = $inputObj->ToXml();
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, $bid, $timeOut);
		$result = WxPayResults::Init($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	...
	{		
		...
		if(!empty($useCert)){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, dirname(__FILE__).'/cert/apiclient_cert'.$useCert.'.pem');
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, dirname(__FILE__).'/cert/apiclient_key'.$useCert.'.pem');
		}
		//post提交方式
