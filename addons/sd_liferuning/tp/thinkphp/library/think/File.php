...
class File extends SplFileObject
{
     ...
     */
    public function move($path, $randcode, $savename = true, $replace = true)
    {
        ...
        // 文件保存命名规则
        $saveName = $this->buildSaveName($savename,$randcode);
     ...
     * @return string
     */
    protected function buildSaveName($savename,$randcode)
    {
        if (true === $savename) {
            // 自动生成文件名
            if ($this->rule instanceof \Closure) {
                ...
                switch ($this->rule) {
                    case 'date':
                        $savename = date('Ym') . DS . date('Ymd') . $randcode . str_replace('.','',microtime(true));
                        break;
                    default:
