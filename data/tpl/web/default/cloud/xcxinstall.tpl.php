...
<table class="table we7-table table-hover">
	...
	<tr>
		...
		<td class="text-left">
			<div class="">
              	<a href="../addons/<?php  echo $row['name'];?>/weimao.php" target="_blank">下载前端</a>
			</div>
		</td>
		<td class="text-left">
			<div class="link-group">
              <?php  if($row['found'] == 0 ) { ?>
				<a href="<?php  echo url('cloud/xcxup');?>m=<?php  echo $row['name'];?>&d=prepare">下载</a>
              <?php  } else { ?>
              	<?php  if($row['version'] > module_ver($row['name']) ) { ?>
              	    <?php  if($row['update_url']=='#' ) { ?>
					<a href="<?php  echo url('cloud/xcxup');?>m=<?php  echo $row['name'];?>&d=prepare">更新模块</a>
					<?php  } else { ?>
					<a href="#">&raquo;&raquo;&raquo;&raquo;&raquo;&raquo;&raquo;</a>
					<?php  } ?>
              	<?php  } else { ?>
              		<a href="<?php  echo url('cloud/xcxup');?>m=<?php  echo $row['id'];?>"></a>
              	<?php  } ?>
              <?php  } ?>
                <?php  if($row['update_url']=='#' ) { ?>
              	<a href="<?php  echo url('cloud/xcxup');?>m=<?php  echo $row['name'];?>&d=prepare" data-toggle="modal" >检测更新</a>
              	<?php  } else { ?>
		...
		<td class="text-left">
			<div class="link-group">
              ...
              <?php  } else if($row['edition']=='1') { ?>
              <a href="##<?php  echo $row['name'];?>" style="color:green" data-toggle="modal" >授权版</a>
              <?php  } else if($row['edition']=='2') { ?>
              <a href="##<?php  echo $row['name'];?>" style="color:red" data-toggle="modal" >VIP免费</a>
              <?php  } ?>
			</div>
		</td>
	</tr>
